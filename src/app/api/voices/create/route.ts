...
const MAX_UOLOAD_SIZE_BYTES = 20 * 1024 * 1024; // 20 MB
...
export async function POST(request: Request) {
  ...
  if (fileBuffer.byteLength > MAX_UOLOAD_SIZE_BYTES) {
    return Response.json(
  ...
  if (duration < MIN_AUDIO_DURATION_SECONDS) {
    return Response.json(
      {
        error: `Audio too short (${duration.toFixed(1)}s). Minimum duraton is ${MIN_AUDIO_DURATION_SECONDS} seconds`,
      },
      { status: 422 },
    );
  }
  ...
  try {
    ...
    const dataObjectKey = `voices/orgs/${orgId}/${voice.id}`;
  ...
  polar.events
    ...
    .catch(() => {});
  return Response.json(
    { name, message: "Voice creatd successfully" },
    { status: 201 },
  );
}
